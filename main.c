...
typedef struct {
    ...
    int level;
    int speed;
    bool game_over;//布尔类型，返回false/true
    bool paused;

    HighScore highscores[MAX_SCORES];//排行榜数组，只有五位
...
void show_game_over_dialog(GameState *game);//函数定义说明
...
void reset_game(GameState *game) {
    free(game->snake.body);//释放内存
    init_game(game);//结束游戏之后就要重新初始化游戏
}
...
void generate_food(GameState *game) {
    ...
    do {
        valid = true;
        game->food.pos.x = rand() % GRID_SIZE; //取模保证生成的食物在游戏画面之内
...
void update_game(GameState *game) {
    if (game->paused || game->game_over) return;//如果二者值只要其一为true,则游戏暂停/结束
    ...
    if (new_head.x < 0 || new_head.x >= GRID_SIZE ||
        ...
        game->game_over = true;
        show_game_over_dialog(game);//展示游戏结束的显示内容
        return;
        }
    //检查蛇头撞蛇身
    for (int i = 0; i < game->snake.length; i++) {
    ...
    if (new_head.x == game->food.pos.x && new_head.y == game->food.pos.y) {
        ...
        // 增加分数，不同关卡加分也不同，第一关加10分，第二关一个食物加20分，以此类推
